...
```{r}
global_waste_data |>
  select(country_name, income_id, total_msw_total_msw_generated_tons_year, population_population_number_of_people)
```
...
composition <- global_waste_data |>
  select(country_name, 
         income_id, 
         total_msw_total_msw_generated_tons_year, 
         population_population_number_of_people, starts_with("composition"))
```
...
#Mutate( ) creates a new column while case_when( ) provides the meaning to the income classes in their specific order.

3.  How is the object `levels_income_cat` used and what does it achieve?

#It has been used to factor out the income levels in a specific order.
...
#The table outlines the income levels in a specific as well the frequency of the observations
...
#The income class are in full compared to the income_id which used abbreviations. They are organized in a specific order.
